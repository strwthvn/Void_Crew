...
void initLogging(std::string_view level, const std::filesystem::path &logFile) {
    ...
    auto fileSink =
        std::make_shared<spdlog::sinks::rotating_file_sink_mt>(logFile.string(), LOG_MAX_FILE_SIZE, LOG_MAX_FILES);

    auto logger = std::make_shared<spdlog::logger>("void_crew", spdlog::sinks_init_list{consoleSink, fileSink});
