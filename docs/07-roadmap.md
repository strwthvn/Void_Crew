...
- Настроить compile-time фильтрацию уровней (TRACE/DEBUG отключены в Release)
- Настроить flush policy (автоматический flush на уровне WARN и выше)
- *Примечание:* синхронный режим (_mt sinks) достаточен для фазы 1; переход на async logger — в фазе 2 (§2.1), когда сетевая нагрузка потребует неблокирующей записи
